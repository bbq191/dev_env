  ...
  (add-hook 'server-after-make-frame-hook
            (lambda ()
              (if (display-graphic-p)
                  (menu-bar-mode 1)
                (menu-bar-mode -1))))
  (add-hook 'vk/after-load-theme-hook
            (lambda ()
...
;; Theme
(if (vk/compatible-theme-p vk-theme)
    (progn
      ...
      (use-package doom-themes
        :bind ("C-c T" . vk/do-load-theme)
        :init (vk/do-load-theme vk-theme t)
        :config
  ...
  (progn
    ...
    (vk/load-theme vk-theme t)))

;; Mode-line
(use-package doom-modeline
  :hook (after-init . doom-modeline-mode)
  :init
  (setq doom-modeline-icon vk-icon
...
(use-package nerd-icons
  ...
  (when (and (display-graphic-p)
             (not (vk/font-installed-p nerd-icons-font-family)))
...
;; Ligatures support
(when (and (not vk-prettify-symbols-alist))
